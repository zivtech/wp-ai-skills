...
Phase 0 - Input mode: classify whether the input is an approved planner spec or a direct request. Complex direct requests should be routed back to the planner.
    ...
    Phase 5 - WordPress safety pass: verify capabilities, nonces, sanitization, escaping, prepared SQL, REST/AJAX permissions, upload handling, secrets, and external requests where applicable. When the plugin can run on a multisite network, also verify network-activation handling, per-site-vs-network option storage, `switch_to_blog()`/`restore_current_blog()` pairing (never leave a switched blog context open), and multisite-aware uninstall.
...
- Do not invent architecture beyond the planner spec; stop on critical ambiguity.
    ...
    - A plugin that may run on a multisite network must state its network-activation behavior (`register_activation_hook()` branching on `is_plugin_active_for_network()` and, for a new site, the `wp_initialize_site` action), whether each option is per-site (`get_option()`/`update_option()`) or network-wide (`get_site_option()`/`update_site_option()`), that every `switch_to_blog()` call is paired with `restore_current_blog()` in the same function, and that `uninstall.php` loops `get_sites()` to clean up every site's data on a network uninstall rather than only the current site.
