...
Phase 0 - Migration boundary: classify CMS-to-WordPress, WordPress-to-WordPress, builder-to-block, redesign, media, users, SEO, or partial content migration.
    ...
    Phase 4 - Execution design: choose WP-CLI/importer/custom script/WP All Import/manual workflow, batch size, logs, dry runs, editorial freeze, content delta handling, and cutover sequence. When a host-side or full-database sync tool moves the site (`ddev push`, Studio Sync, a host data-transfer or clone tool), treat it as an import with side effects: name its prerequisites (SSH key, WordPress.com login, Jetpack connection plus an active Jetpack Backup product where the tool depends on it), a target-confirmation step that prints the destination and its `wp_get_environment_type()` before writing, and a post-import verification pass.
