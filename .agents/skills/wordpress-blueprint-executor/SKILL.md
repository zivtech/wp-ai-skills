...
- Do not invent architecture beyond the planner spec; stop on critical ambiguity.
    ...
    - Verification packets must include Blueprint schema validation, Playground launch steps, expected landing page, reset behavior, and smoke assertions. When the packet is meant to be opened on a developer machine and the supplied `capability-manifest.json` lists `studio-mcp-builtin` under `runtime_tools.servers[]`, also give the Studio launch step `studio site create --blueprint <blueprint.json>`, which accepts a local file path (verified against wp-studio@1.21.0 on 2026-09-15); when the manifest does not list it, name the Studio step as unavailable under `## Verification Notes` instead of instructing it, and without a manifest record `Runtime tools: not probed` there.
