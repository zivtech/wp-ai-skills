...
- Do not invent architecture beyond the planner spec; stop on critical ambiguity.
    ...
    - Any literal serialized block markup this executor writes (saved-content fixtures, deprecated-version snapshots, demo/starter content) must be produced through `@wordpress/blocks`' `serialize()` output, not hand-typed HTML comment delimiters and attributes; hand-written block markup is a common source of a block the Site Editor rejects even though a static artifact check accepted it. Run generated markup through `parse_blocks()`/`validateBlock` (or an editor insert-save-reload round trip) as a required block-validation gate before treating it as done, not an optional check.
