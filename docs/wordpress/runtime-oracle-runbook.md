...
Two further checks run under the same flag against the manifest's `runtime_tools` section (schema 1.1.0, recorded by the probe from config and `--help` text only, never by executing a runtime tool):

- `runtime_tool_grounding` (weight 3) scans code occurrences for `ddev push pressable`, `ddev pull pressable`, `studio site create`, `studio mcp`, the standalone `studio-mcp`, a bare Studio MCP tool name such as `site_push`, or the citation form `runtime_tools.<server-id>.<tool>`. An instruction fails when `runtime_tools.status` is `UNAVAILABLE`, when the server is not in `runtime_tools.servers[]`, when the server is marked `superseded`, or when the server does not list the tool. A manifest that predates `runtime_tools` reports `not probed` as an unresolved state. `wp_cli` is the one tool name that grounds only through the citation form, because it is also the manifest's own top-level key and appears in ordinary prose.
- `runtime_sync_confirmation` (weight 3) applies to instructed tools whose `side_effect` is `inward_sync` or `outward_sync`. The level-two section that gives the instruction must carry `Sync tool:` equal to the manifest's tool name, a usable `Sync target:` that honours the tool's `target_constraint` (`staging-only` rejects a target that omits `staging` or reads as production), and `wp_get_environment_type()` as the pre-write confirmation. Provisioning a disposable local site has no remote destination to confirm and is grounding-only.
...
Current sidecars: `wordpress-planner.migration/fixtures/host-sync-staging-cutover-v1` (ddev-pressable, so `ddev push pressable` must be instructed with `Sync tool:`, a staging `Sync target:`, `wp_get_environment_type()`, and the `ddev wp` prefix) and `wordpress-blueprint-executor/fixtures/studio-launch-handoff-v1` (studio-mcp-builtin, so `studio site create --blueprint` grounds and any sync tool or standalone `studio-mcp` fails). First recorded run against the migration fixture: `evidence/wordpress-high-risk-evals/wordpress-planner-migration-sidecar-saved-outputs-20260916/` (skill = Sonnet low effort, baselines = `codex exec` gpt-5.5). All three lanes generated; none passed the contract. The skill output is grounded (`ddev wp` prefix, only manifest-listed tools, `Runtime tools:` from the manifest) but fails `runtime_sync_confirmation` twice: it put `Sync tool:` / `Sync target:` inside a numbered step, which the record parser deliberately does not read, and it instructed `ddev pull pressable` as the baseline without a record of its own. Both baselines instructed sync tools with no confirmation records; the few-shot baseline also instructed Studio `site_push` / `site_pull` and the superseded `studio-mcp`, none of which the manifest lists. That run is n=1 per lane and is contract evidence only, not a quality comparison. First recorded run against the blueprint fixture: `evidence/wordpress-high-risk-evals/wordpress-blueprint-executor-sidecar-saved-outputs-20260916/`, produced in the new single-stage `executor-direct` invocation mode (the three-stage pipeline fed the fixture to a planner that refused it). The skill lane produced a real packet that gives the grounded `studio site create --blueprint blueprint.json` step, then fails on five missing packet headings, a bare `wp plugin` where the manifest prefix is `studio wp`, the superseded standalone `studio-mcp`, and Studio `preview_create` / `site_push` instructed with no confirmation records. Both baselines also instructed the superseded `studio-mcp`. What it does not establish: whether the record-placement failure is a prompt-clarity gap (the migration skill never says a record must be its own unindented line) or a model error; that is the next thing to test before touching the parity-locked skill text.
